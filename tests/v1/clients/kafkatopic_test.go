...
func NewTopic() *kafkav1.KafkaTopic {
	replicationFactor := int32(3)
	partitions := int32(3)

	return &kafkav1.KafkaTopic{
		...
		Spec: &kafkav1.KafkaTopicSpec{
			...
			Config:     kafkav1.MapStringObject{"retention.ms": 7200000, "segment.bytes": 1073741824},
		},
	}
}

func UpdatedTopic(oldResource *kafkav1.KafkaTopic) *kafkav1.KafkaTopic {
...
func AssertTopics(t *testing.T, r1, r2 *kafkav1.KafkaTopic) {
...
func TestKafkaTopicCreateUpdateDelete(t *testing.T) {
	...
	defer client.KafkaV1().KafkaTopics(NAMESPACE).Delete(context.TODO(), NAME, metav1.DeleteOptions{})

	// Create the resource
	newResource := NewTopic()
	_, err := client.KafkaV1().KafkaTopics(NAMESPACE).Create(context.TODO(), newResource, metav1.CreateOptions{})
	if err != nil {
		t.Fatalf("Failed to create actualResource: %s", err.Error())
	}

	// Get the resource
	actualResource, err := client.KafkaV1().KafkaTopics(NAMESPACE).Get(context.TODO(), NAME, metav1.GetOptions{})
	if err != nil {
	...
	_, err = client.KafkaV1().KafkaTopics(NAMESPACE).Update(context.TODO(), updatedResource, metav1.UpdateOptions{})
	if err != nil {
	...
	// Delete the resource
	err = client.KafkaV1().KafkaTopics(NAMESPACE).Delete(context.TODO(), NAME, metav1.DeleteOptions{})
	if err != nil {
...
func TestKafkaTopicInformerAndLister(t *testing.T) {
	...
	informer := factory.Kafka().V1().KafkaTopics()
