...
func NewKafka() *kafkav1.Kafka {
	return &kafkav1.Kafka{
...
func UpdatedKafka(oldResource *kafkav1.Kafka) *kafkav1.Kafka {
	updatedResource := oldResource.DeepCopy()

	updatedResource.Spec.Kafka.Version = "4.0.0"
	updatedResource.Spec.Kafka.Listeners = append(updatedResource.Spec.Kafka.Listeners, kafkav1.GenericKafkaListener{
		Name: "external",
		Type: kafkav1.LOADBALANCER_KAFKALISTENERTYPE,
		Tls:  true,
		Port: 9094,
	})
...
func AssertKafkas(t *testing.T, r1, r2 *kafkav1.Kafka) {
...
func TestKafkaCreateUpdateDelete(t *testing.T) {
	...
	defer client.KafkaV1().Kafkas(NAMESPACE).Delete(context.TODO(), NAME, metav1.DeleteOptions{})

	// Create the resource
	newResource := NewKafka()
	_, err := client.KafkaV1().Kafkas(NAMESPACE).Create(context.TODO(), newResource, metav1.CreateOptions{})
	if err != nil {
		t.Fatalf("Failed to create actualResource: %s", err.Error())
	}

	// Get the resource
	actualResource, err := client.KafkaV1().Kafkas(NAMESPACE).Get(context.TODO(), NAME, metav1.GetOptions{})
	if err != nil {
	...
	_, err = client.KafkaV1().Kafkas(NAMESPACE).Update(context.TODO(), updatedResource, metav1.UpdateOptions{})
	if err != nil {
	...
	// Delete the resource
	err = client.KafkaV1().Kafkas(NAMESPACE).Delete(context.TODO(), NAME, metav1.DeleteOptions{})
	if err != nil {
...
func TestKafkaInformerAndLister(t *testing.T) {
	...
	informer := factory.Kafka().V1().Kafkas()
