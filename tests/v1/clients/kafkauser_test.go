...
func NewUser() *kafkav1.KafkaUser {
	return &kafkav1.KafkaUser{
		ObjectMeta: metav1.ObjectMeta{
			Name:      NAME,
			Namespace: NAMESPACE,
		},
		Spec: &kafkav1.KafkaUserSpec{
			Authentication: &kafkav1.KafkaUserAuthentication{
				Type: kafkav1.TLS_KAFKAUSERAUTHENTICATIONTYPE,
			},
			Authorization: &kafkav1.KafkaUserAuthorization{
				Type: kafkav1.SIMPLE_KAFKAUSERAUTHORIZATIONTYPE,
				Acls: []kafkav1.AclRule{{
					Operations: []kafkav1.AclOperation{kafkav1.READ_ACLOPERATION, kafkav1.DESCRIBE_ACLOPERATION},
					Resource:   &kafkav1.AclRuleResource{Name: "my-topic", Type: kafkav1.TOPIC_ACLRULERESOURCETYPE},
				}},
			},
		},
	}
...
func UpdatedUser(oldResource *kafkav1.KafkaUser) *kafkav1.KafkaUser {
	updatedResource := oldResource.DeepCopy()

	updatedResource.Spec.Authentication = &kafkav1.KafkaUserAuthentication{
		Type: kafkav1.SCRAM_SHA_512_KAFKAUSERAUTHENTICATIONTYPE,
	}

	updatedResource.Spec.Authorization = &kafkav1.KafkaUserAuthorization{
		Type: kafkav1.SIMPLE_KAFKAUSERAUTHORIZATIONTYPE,
		Acls: []kafkav1.AclRule{{
			Operations: []kafkav1.AclOperation{kafkav1.WRITE_ACLOPERATION, kafkav1.DESCRIBE_ACLOPERATION},
			Resource:   &kafkav1.AclRuleResource{Name: "my-other-topic", Type: kafkav1.TOPIC_ACLRULERESOURCETYPE},
		}},
	}
...
func AssertUsers(t *testing.T, r1, r2 *kafkav1.KafkaUser) {
...
func TestKafkaUserCreateUpdateDelete(t *testing.T) {
	...
	defer client.KafkaV1().KafkaUsers(NAMESPACE).Delete(context.TODO(), NAME, metav1.DeleteOptions{})

	// Create the resource
	newResource := NewUser()
	_, err := client.KafkaV1().KafkaUsers(NAMESPACE).Create(context.TODO(), newResource, metav1.CreateOptions{})
	if err != nil {
		t.Fatalf("Failed to create actualResource: %s", err.Error())
	}

	// Get the resource
	actualResource, err := client.KafkaV1().KafkaUsers(NAMESPACE).Get(context.TODO(), NAME, metav1.GetOptions{})
	if err != nil {
	...
	_, err = client.KafkaV1().KafkaUsers(NAMESPACE).Update(context.TODO(), updatedResource, metav1.UpdateOptions{})
	if err != nil {
	...
	// Delete the resource
	err = client.KafkaV1().KafkaUsers(NAMESPACE).Delete(context.TODO(), NAME, metav1.DeleteOptions{})
	if err != nil {
...
func TestKafkaUserInformerAndLister(t *testing.T) {
	...
	informer := factory.Kafka().V1().KafkaUsers()
