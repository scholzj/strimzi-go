...
func NewRebalance() *kafkav1.KafkaRebalance {
	return &kafkav1.KafkaRebalance{
		ObjectMeta: metav1.ObjectMeta{
			Name:      NAME,
			Namespace: NAMESPACE,
		},
		Spec: &kafkav1.KafkaRebalanceSpec{
			Goals: []string{"CpuCapacityGoal", "DiskCapacityGoal"},
		},
	}
}

func UpdatedRebalance(oldResource *kafkav1.KafkaRebalance) *kafkav1.KafkaRebalance {
...
func AssertRebalances(t *testing.T, r1, r2 *kafkav1.KafkaRebalance) {
...
func TestKafkaRebalanceCreateUpdateDelete(t *testing.T) {
	...
	defer client.KafkaV1().KafkaRebalances(NAMESPACE).Delete(context.TODO(), NAME, metav1.DeleteOptions{})

	// Create the resource
	newResource := NewRebalance()
	_, err := client.KafkaV1().KafkaRebalances(NAMESPACE).Create(context.TODO(), newResource, metav1.CreateOptions{})
	if err != nil {
		t.Fatalf("Failed to create actualResource: %s", err.Error())
	}

	// Get the resource
	actualResource, err := client.KafkaV1().KafkaRebalances(NAMESPACE).Get(context.TODO(), NAME, metav1.GetOptions{})
	if err != nil {
	...
	_, err = client.KafkaV1().KafkaRebalances(NAMESPACE).Update(context.TODO(), updatedResource, metav1.UpdateOptions{})
	if err != nil {
	...
	// Delete the resource
	err = client.KafkaV1().KafkaRebalances(NAMESPACE).Delete(context.TODO(), NAME, metav1.DeleteOptions{})
	if err != nil {
...
func TestKafkaRebalanceInformerAndLister(t *testing.T) {
	...
	informer := factory.Kafka().V1().KafkaRebalances()
