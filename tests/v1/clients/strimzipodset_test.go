...
func NewPodSet() *corev1.StrimziPodSet {
	return &corev1.StrimziPodSet{
		ObjectMeta: metav1.ObjectMeta{
			Name:      NAME,
			Namespace: NAMESPACE,
		},
		Spec: &corev1.StrimziPodSetSpec{
			Selector: &metav1.LabelSelector{
				MatchLabels: map[string]string{
					"strimzi.io/label": "value",
				},
			},
			Pods: []kafkav1.MapStringObject{{
				"apiVersion": "v1",
				"kind":       "Pod",
...
func UpdatedPodSet(oldResource *corev1.StrimziPodSet) *corev1.StrimziPodSet {
	updatedResource := oldResource.DeepCopy()

	updatedResource.Spec.Selector.MatchLabels["strimzi.io/label"] = "value2"

	updatedPods := []kafkav1.MapStringObject{{
		"apiVersion": "v1",
		"kind":       "Pod",
		"metadata": map[string]interface{}{
			"labels": map[string]interface{}{
...
func AssertPodSets(t *testing.T, r1, r2 *corev1.StrimziPodSet) {
...
func TestPodSetCreateUpdateDelete(t *testing.T) {
	...
	defer client.CoreV1().StrimziPodSets(NAMESPACE).Delete(context.TODO(), NAME, metav1.DeleteOptions{})

	// Create the resource
	newResource := NewPodSet()
	_, err := client.CoreV1().StrimziPodSets(NAMESPACE).Create(context.TODO(), newResource, metav1.CreateOptions{})
	if err != nil {
		t.Fatalf("Failed to create actualResource: %s", err.Error())
	}

	// Get the resource
	actualResource, err := client.CoreV1().StrimziPodSets(NAMESPACE).Get(context.TODO(), NAME, metav1.GetOptions{})
	if err != nil {
	...
	_, err = client.CoreV1().StrimziPodSets(NAMESPACE).Update(context.TODO(), updatedResource, metav1.UpdateOptions{})
	if err != nil {
	...
	// Delete the resource
	err = client.CoreV1().StrimziPodSets(NAMESPACE).Delete(context.TODO(), NAME, metav1.DeleteOptions{})
	if err != nil {
...
func TestPodSetInformerAndLister(t *testing.T) {
	...
	informer := factory.Core().V1().StrimziPodSets()
