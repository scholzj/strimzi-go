...
func NewConnect() *kafkav1.KafkaConnect {
	return &kafkav1.KafkaConnect{
		ObjectMeta: metav1.ObjectMeta{
			Name:      NAME,
			Namespace: NAMESPACE,
		},
		Spec: &kafkav1.KafkaConnectSpec{
			Replicas:           1,
...
func UpdatedConnect(oldResource *kafkav1.KafkaConnect) *kafkav1.KafkaConnect {
...
func AssertConnects(t *testing.T, r1, r2 *kafkav1.KafkaConnect) {
...
func TestKafkaConnectCreateUpdateDelete(t *testing.T) {
	...
	defer client.KafkaV1().KafkaConnects(NAMESPACE).Delete(context.TODO(), NAME, metav1.DeleteOptions{})

	// Create the resource
	newResource := NewConnect()
	_, err := client.KafkaV1().KafkaConnects(NAMESPACE).Create(context.TODO(), newResource, metav1.CreateOptions{})
	if err != nil {
		t.Fatalf("Failed to create actualResource: %s", err.Error())
	}

	// Get the resource
	actualResource, err := client.KafkaV1().KafkaConnects(NAMESPACE).Get(context.TODO(), NAME, metav1.GetOptions{})
	if err != nil {
	...
	_, err = client.KafkaV1().KafkaConnects(NAMESPACE).Update(context.TODO(), updatedResource, metav1.UpdateOptions{})
	if err != nil {
	...
	// Delete the resource
	err = client.KafkaV1().KafkaConnects(NAMESPACE).Delete(context.TODO(), NAME, metav1.DeleteOptions{})
	if err != nil {
...
func TestKafkaConnectInformerAndLister(t *testing.T) {
	...
	informer := factory.Kafka().V1().KafkaConnects()
