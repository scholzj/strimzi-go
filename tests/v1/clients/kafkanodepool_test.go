...
func NewNodePool() *kafkav1.KafkaNodePool {
	volumeId := int32(0)

	return &kafkav1.KafkaNodePool{
...
func UpdatedNodePool(oldResource *kafkav1.KafkaNodePool) *kafkav1.KafkaNodePool {
	updatedResource := oldResource.DeepCopy()

	updatedResource.Spec.Replicas = 5
	updatedResource.Spec.Roles = []kafkav1.ProcessRoles{kafkav1.CONTROLLER_PROCESSROLES}
	updatedResource.Spec.Storage.Volumes[0].Size = "1Ti"

	return updatedResource
}

func AssertNodePools(t *testing.T, r1, r2 *kafkav1.KafkaNodePool) {
...
func TestKafkaNodePoolCreateUpdateDelete(t *testing.T) {
	...
	defer client.KafkaV1().KafkaNodePools(NAMESPACE).Delete(context.TODO(), NAME, metav1.DeleteOptions{})

	// Create the resource
	newResource := NewNodePool()
	_, err := client.KafkaV1().KafkaNodePools(NAMESPACE).Create(context.TODO(), newResource, metav1.CreateOptions{})
	if err != nil {
		t.Fatalf("Failed to create actualResource: %s", err.Error())
	}

	// Get the resource
	actualResource, err := client.KafkaV1().KafkaNodePools(NAMESPACE).Get(context.TODO(), NAME, metav1.GetOptions{})
	if err != nil {
	...
	_, err = client.KafkaV1().KafkaNodePools(NAMESPACE).Update(context.TODO(), updatedResource, metav1.UpdateOptions{})
	if err != nil {
	...
	// Delete the resource
	err = client.KafkaV1().KafkaNodePools(NAMESPACE).Delete(context.TODO(), NAME, metav1.DeleteOptions{})
	if err != nil {
...
func TestKafkaNodePoolInformerAndLister(t *testing.T) {
	...
	informer := factory.Kafka().V1().KafkaNodePools()
