...
type KafkaConnectLister interface {
	...
	List(selector labels.Selector) (ret []*kafkastrimziiov1.KafkaConnect, err error)
...
type kafkaConnectLister struct {
	listers.ResourceIndexer[*kafkastrimziiov1.KafkaConnect]
}
...
func NewKafkaConnectLister(indexer cache.Indexer) KafkaConnectLister {
	return &kafkaConnectLister{listers.New[*kafkastrimziiov1.KafkaConnect](indexer, kafkastrimziiov1.Resource("kafkaconnect"))}
}

// KafkaConnects returns an object that can list and get KafkaConnects.
func (s *kafkaConnectLister) KafkaConnects(namespace string) KafkaConnectNamespaceLister {
	return kafkaConnectNamespaceLister{listers.NewNamespaced[*kafkastrimziiov1.KafkaConnect](s.ResourceIndexer, namespace)}
}
...
type KafkaConnectNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*kafkastrimziiov1.KafkaConnect, error)
