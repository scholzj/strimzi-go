...
type KafkaConnectorLister interface {
	...
	List(selector labels.Selector) (ret []*kafkastrimziiov1.KafkaConnector, err error)
...
type kafkaConnectorLister struct {
	listers.ResourceIndexer[*kafkastrimziiov1.KafkaConnector]
}
...
func NewKafkaConnectorLister(indexer cache.Indexer) KafkaConnectorLister {
	return &kafkaConnectorLister{listers.New[*kafkastrimziiov1.KafkaConnector](indexer, kafkastrimziiov1.Resource("kafkaconnector"))}
}

// KafkaConnectors returns an object that can list and get KafkaConnectors.
func (s *kafkaConnectorLister) KafkaConnectors(namespace string) KafkaConnectorNamespaceLister {
	return kafkaConnectorNamespaceLister{listers.NewNamespaced[*kafkastrimziiov1.KafkaConnector](s.ResourceIndexer, namespace)}
}
...
type KafkaConnectorNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*kafkastrimziiov1.KafkaConnector, error)
