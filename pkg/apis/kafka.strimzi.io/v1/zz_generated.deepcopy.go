...
func (in *GenericKafkaListenerConfiguration) DeepCopyInto(out *GenericKafkaListenerConfiguration) {
	*out = *in
	...
	if in.PerBrokerAnnotationsTemplate != nil {
		in, out := &in.PerBrokerAnnotationsTemplate, &out.PerBrokerAnnotationsTemplate
		*out = make(map[string]string, len(*in))
		for key, val := range *in {
			(*out)[key] = val
		}
	}
	if in.PerBrokerLabelsTemplate != nil {
		in, out := &in.PerBrokerLabelsTemplate, &out.PerBrokerLabelsTemplate
		*out = make(map[string]string, len(*in))
		for key, val := range *in {
			(*out)[key] = val
		}
